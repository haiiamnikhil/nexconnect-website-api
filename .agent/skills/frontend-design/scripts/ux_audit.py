...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        is_html = filename.endswith('.html') or filename.endswith('.vue') or filename.endswith('.svelte')
        has_long_text = bool(re.search(r'<p|<div.*class=.*text|article|<span.*text', content, re.IGNORECASE))
        has_form = is_html and bool(re.search(r'<form\b|<input\b', content, re.IGNORECASE))
        ...
        if form_fields > 7 and is_html and not re.search(r'step|wizard|stage', content, re.IGNORECASE):
